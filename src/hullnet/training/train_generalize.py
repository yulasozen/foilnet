"""Train HullGNN on multiple hull graphs and evaluate generalization to a held-out hull."""
...
def main() -> None:
    ...
    parser.add_argument(
        "test_hull", nargs="?", default="wigley_02",
        help="Hull id held out for testing (default: wigley_02)",
    )
    ...
    if args.test_hull not in hulls:
        raise SystemExit(f"test hull '{args.test_hull}' not found among {sorted(hulls)}")

    test_id = args.test_hull
    train_ids = [h for h in hulls if h != test_id]
    print(f"train hulls: {train_ids}")
    print(f"test hull:   {test_id}")

    train_data = [hulls[h].to(device) for h in train_ids]
    test_data = hulls[test_id].to(device)

    # Fit scalers on pooled TRAINING data only -- test hull never touches this fit.
    ...
    with torch.no_grad():
        ...
        x_test, y_test = scale(test_data)
        test_pred = model(x_test, test_data.edge_index)
        test_mse = loss_fn(test_pred, y_test).item()
        test_pred_unscaled = test_pred * y_std + y_mean
        test_r2 = r2_score(test_pred_unscaled, test_data.y)

    print(f"\ntrain R^2 (original units, pooled over {len(train_ids)} hulls): {train_r2:.6f}")
    print(f"test hull:                 {test_id}")
    print(f"test MSE (standardized):   {test_mse:.6f}")
    print(f"test R^2 (original units): {test_r2:.6f}")
